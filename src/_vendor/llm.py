...
Vendored from the Resume Matcher backend (apps/backend/app/llm.py) and trimmed:
the original imported `app.config.settings` to load a JSON-backed config file
used by the web app. This tool always constructs `LLMConfig` explicitly from
its own env vars (see src/cli.py:_build_llm_config), so the settings-dependent
helpers (`get_llm_config`, `resolve_api_key`, `_load_stored_config`) and the
`_PROVIDER_KEY_MAP` table are removed. `get_router()` and `check_llm_health()`
now require an explicit `LLMConfig` argument.
"""
...
def get_router(config: LLMConfig) -> tuple[Router, LLMConfig]:
...
async def complete(
    ...
    if config is None:
        raise ValueError("complete() requires an explicit LLMConfig in this vendored build")
...
async def complete_json(
    ...
    if config is None:
        raise ValueError("complete_json() requires an explicit LLMConfig in this vendored build")
